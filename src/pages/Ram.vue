...
<template>
  <div class="container mt-4">
    <h2 class="letra1">Gestión de Memoria Ram</h2>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Memoria Ram' : 'Agregar Memoria Ram' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              <label for="nombre_ram">Capacidad de Memoria Ram</label>
              <input 
                type="text"
                id="nombre_ram"
                v-model="formData.nombre_ram"
                class="form-control"
                placeholder="Capacidad de Memoria Ram"
                required
              />
            </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="ram in ram" :key="ram.id_ram">
            <td>{{ ram.id_ram }}</td>
            <td>{{ ram.nombre_ram }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(ram)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(ram.id_ram)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_ram) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/ram/${this.formData.id_ram}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/ram`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
        console.error("Error al guardar la Memoria Ram:", error);
      }
    },
    // Eliminar una agencia
    async eliminarAgencia(id) {
      if (confirm("¿Estás seguro de eliminar esta Memoria Ram?")) {
        try {
          await axios.delete(`${apiUrl}/ram/${id}`);
          this.fetchAgencias();
        } catch (error) {
