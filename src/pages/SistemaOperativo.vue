  ...
  <div class="container mt-4">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Sistema Operativo' : 'Agregar Sistema Operativo' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              <label for="nombre_so">Nombre Sistema Operativo</label>
              <input 
                type="text"
                id="nombre_so"
                v-model="formData.nombre_so"
                class="form-control"
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="so in sos" :key="so.id_so">
            <td>{{ so.id_so }}</td>
            <td>{{ so.nombre_so }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(so)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(so.id_so)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_so) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/so/${this.formData.id_so}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/so`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
        console.error("Error al guardar el Sistema Operativo:", error);
      }
    },
    ...
    async eliminarAgencia(id) {
      if (confirm("¿Estás seguro de eliminar este Sistema Operativo?")) {
        try {
          await axios.delete(`${apiUrl}/so/${id}`);
          this.fetchAgencias();
        } catch (error) {
