...
<template>
  <div class="container mt-4">
    <h2 class="letra1">Gestión de Office</h2>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Office' : 'Agregar Office' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              <label for="nombre_office">Version Office</label>
              <input 
                type="text"
                id="nombre_office"
                v-model="formData.nombre_office"
                class="form-control"
                placeholder="Version del Office"
                required
              />
            </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="office in office" :key="office.id_office">
            <td>{{ office.id_office }}</td>
            <td>{{ office.nombre_office }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(office)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(office.id_office)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_office) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/office/${this.formData.id_office}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/office`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
        console.error("Error al guardar el Office:", error);
      }
    },
    // Eliminar una agencia
    async eliminarAgencia(id) {
      if (confirm("¿Estás seguro de eliminar este Office?")) {
        try {
