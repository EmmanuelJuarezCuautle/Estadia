...
<template>
  <div class="container mt-4">
    <h2 class="letra1">Gestión de Monitores</h2>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Monitor' : 'Agregar Monitor' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              <label for="nombre_monitor">Nombre Monitor</label>
              <input 
                type="text"
                id="nombre_monitor"
                v-model="formData.nombre_monitor"
                class="form-control"
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="monitor in monitores" :key="monitor.id_monitor">
            <td>{{ monitor.id_monitor }}</td>
            <td>{{ monitor.nombre_monitor }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(monitor)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(monitor.id_monitor)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_monitor) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/monitor/${this.formData.id_monitor}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/monitor`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
        console.error("Error al guardar la monitor:", error);
      }
    },
    ...
    async eliminarAgencia(id) {
      if (confirm("¿Estás seguro de eliminar este monitor?")) {
        try {
          await axios.delete(`${apiUrl}/monitor/${id}`);
          this.fetchAgencias();
        } catch (error) {
