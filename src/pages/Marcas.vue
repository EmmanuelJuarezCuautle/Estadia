      ...
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="marca in marcas" :key="marca.id_marca">
            ...
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(marca)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(marca.id_marca)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_marca) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/marca/${this.formData.id_marca}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/marca`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
