...
<template>
  <div class="container mt-4">
    <h2 class="letra1">Gestión de Antivirus</h2>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Antivirus' : 'Agregar Antivirus' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              <label for="nombre_antivirus">Nombre Antivirus</label>
              <input 
                type="text"
                id="nombre_antivirus"
                v-model="formData.nombre_antivirus"
                class="form-control"
                placeholder="Nombre del antivirus"
                required
              />
            </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="antivirus in antivirus" :key="antivirus.id_antivirus">
            <td>{{ antivirus.id_antivirus }}</td>
            <td>{{ antivirus.nombre_antivirus }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(antivirus)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(antivirus.id_antivirus)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_antivirus) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/antivirus/${this.formData.id_antivirus}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/antivirus`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
        console.error("Error al guardar el antivirus:", error);
      }
    },
    // Eliminar una agencia
    async eliminarAgencia(id) {
      if (confirm("¿Estás seguro de eliminar este antivirus?")) {
        try {
          await axios.delete(`${apiUrl}/antivirus/${id}`);
          this.fetchAgencias();
        } catch (error) {
          console.error("Error al eliminar antivirus:", error);
        }
      }
    },
