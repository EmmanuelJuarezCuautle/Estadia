<template>
  <div class="container mt-4">
    <h2 class="letra1">Gestión de Departamentos</h2>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="departamento in departamentos" :key="departamento.id_depto">
            <td>{{ departamento.id_depto }}</td>
            <td>{{ departamento.nombre_depto }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(departamento)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(departamento.id_depto)">
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_depto) {
          ...
          await axios.put(`${apiUrl}/departamento/${this.formData.id_depto}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/departamento`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
        console.error("Error al guardar la depto:", error);
      }
    },
