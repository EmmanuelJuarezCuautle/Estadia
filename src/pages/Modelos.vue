...
<template>
  <div class="container mt-4">
    <h2 class="letra1">Gestión de Modelos</h2>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Modelo' : 'Agregar Modelo' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              ...
              <input 
                type="text"
                id="nombre_modelo"
                v-model="formData.nombre_modelo"
                class="form-control"
      ...
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="modelo in modelos" :key="modelo.id_modelo">
            ...
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(modelo)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(modelo.id_modelo)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_modelo) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/modelo/${this.formData.id_modelo}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/modelo`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
