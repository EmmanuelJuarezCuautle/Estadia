  ...
  <div class="container mt-4">
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Categoria' : 'Agregar Categoria' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              <label for="nombre_catego">Nombre Categoria</label>
              <input 
                type="text"
                id="nombre_catego"
                v-model="formData.nombre_catego"
                class="form-control"
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="categoria in categorias" :key="categoria.id_catego">
            <td>{{ categoria.id_catego }}</td>
            <td>{{ categoria.nombre_catego }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(categoria)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(categoria.id_catego)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_catego) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/categoria/${this.formData.id_catego}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/categoria`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
    ...
    async eliminarAgencia(id) {
      if (confirm("¿Estás seguro de eliminar esta categoria?")) {
        try {
          await axios.delete(`${apiUrl}/categoria/${id}`);
          this.fetchAgencias();
        } catch (error) {
