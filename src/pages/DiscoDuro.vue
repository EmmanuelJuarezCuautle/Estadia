...
<template>
  <div class="container mt-4">
    <h2 class="letra1">Gestión de Discos Duros</h2>
    ...
    <div v-if="showForm" class="modal-overlay" @click.self="cancelForm">
      <div class="card modal-content">
        <div class="card-header">
          <h4 class="mb-4">{{ editMode ? 'Editar Disco Duro' : 'Agregar Disco Duro' }}</h4>
        </div>
        <div class="card-body">
          <form @submit.prevent="guardarAgencia">
            <div class="form-group">
              <label for="nombre_dd">Capacidad Disco Duro</label>
              <input 
                type="text"
                id="nombre_dd"
                v-model="formData.nombre_dd"
                class="form-control"
                placeholder="Capacidad del Disco Duro"
                required
              />
            </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-hover text-center">
        ...
        <tbody>
          <tr v-for="discoduro in discoduro" :key="discoduro.id_dd">
            <td>{{ discoduro.id_dd }}</td>
            <td>{{ discoduro.nombre_dd }}</td>
            <td class="td-actions">
              <button class="btn btn-warning btn-sm" style="width: 100px; margin-left: 30px; margin-right: 30px;" @click="editAgency(discoduro)">
                <i class="fa-solid fa-pen-to-square"style="margin-right: 10px;" ></i> Editar
              </button>
              <button class="btn btn-danger btn-sm"  style="width: 100px;" @click="eliminarAgencia(discoduro.id_dd)">
...
export default {
  ...
  methods: {
    ...
    async guardarAgencia() {
      try {
        if (this.formData.id_dd) {
          // Actualizar agencia existente
          await axios.put(`${apiUrl}/discoduro/${this.formData.id_dd}`, this.formData);
        } else {
          // Crear nueva agencia
          await axios.post(`${apiUrl}/discoduro`, this.formData); // Usar backticks aquí
        }
        this.fetchAgencias();
        this.cancelForm();
      } catch (error) {
        console.error("Error al guardar el Disco Duro:", error);
      }
    },
    // Eliminar una agencia
    async eliminarAgencia(id) {
      if (confirm("¿Estás seguro de eliminar este Disco Duro?")) {
        try {
          await axios.delete(`${apiUrl}/discoduro/${id}`);
          this.fetchAgencias();
        } catch (error) {
          console.error("Error al eliminar discoduro:", error);
        }
      }
    },
